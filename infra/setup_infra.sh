...
workdir_names=($teamcity_server_workdir $teamcity_server_agent $selenoid_workdir)
...
#################################################
echo "Stop and delete previous run containers"
...
mkdir config
copy $teamcity-tests-directory/infra/browsers.json config/
docker run -d                                               \
...
echo "Pull browser images: $image_names"
...
echo "host=$ip:8111\nsuperUserToken=$superuser_token\nremote=true" > $teamcity-tests-directory/src/main/resources/config.properties

cat $teamcity-tests-directory/src/main/resources/config.properties
